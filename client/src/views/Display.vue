<template>
  <div
    class="w-full flex top-0 z-10 border-solid border-r-0 border-l-0 border border-t-0 border-grey-light"
  >
  ...
  <form @submit.prevent="submitTweet">
    <div class="pb-4">
      ...
      <div class="w-full flex items-top p-2 ml-12 pl-14">
        <a href="#" class="rounded-full p-2 text-blue">
        ...
        <button
          @click="postTweet"
          class="ml-auto mr-20 w-24 h-10 bg-blue font-bold text-white rounded-full text-center border-0"
        >
          Post
        </button>
      </div>
    ...
    >
      <div class="text-blue-dark text-center">Show 99 posts</div>
    </div>
  </form>
  ...
  <div>
    <Tweet
      v-for="(tweet, index) in tweetStore.tweets"
      :key="index"
      :tweet="tweet"
    />
    <!-- /Tweet -->
